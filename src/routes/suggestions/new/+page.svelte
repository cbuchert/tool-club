...
<div class="p-4 sm:p-6 max-w-xl">
	...
	<p class="mb-6 text-[0.8125rem] text-tc-muted leading-relaxed">
		...
		<form.Field name="body_md">
			{#snippet children(field)}
				...
				<textarea
					...
					oninput={(e) => field.handleChange(e.currentTarget.value)}
					onblur={field.handleBlur}
					class="w-full resize-y rounded-md [border:0.5px_solid_var(--tc-border-mid)] bg-tc-bg px-3 py-2.5 text-sm text-tc-text outline-none transition-colors focus:[border-color:var(--tc-accent-border)] leading-relaxed mb-1.5"
				></textarea>
				{#if field.state.meta.isTouched && field.state.meta.errors.length}
					<p class="mb-3 text-xs text-tc-danger">
						{String(field.state.meta.errors[0])}
		...
		<form.Field name="host_name">
			{#snippet children(field)}
				<label
					...
					class="block font-mono text-[0.6875rem] uppercase tracking-[0.06em] text-tc-hint mb-1.5"
				>
					Nominated host <span class="text-tc-hint font-sans normal-case text-[0.6875rem]"
						>(optional)</span
					>
				</label>
				<input
		...
		<form.Subscribe selector={(s) => ({ canSubmit: s.canSubmit, isSubmitting: s.isSubmitting })}>
			{#snippet children(state)}
				<button
					...
					class="rounded-md bg-tc-accent px-5 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-[0.88] disabled:opacity-50 disabled:cursor-not-allowed"
				>
